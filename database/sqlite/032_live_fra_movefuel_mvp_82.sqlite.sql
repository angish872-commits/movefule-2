...
-- Project: fra / movefule
-- Database: movefuel_mvp
-- Tables: 82
-- Dialect: SQLite
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_calendar_entry_calendar_entry_rev_identity_uq] ON [calendar_entry] ([calendarRevisionId], [entryId]);
...
CREATE INDEX IF NOT EXISTS [ix_calendar_entry_calendar_entry_user_sem_rev] ON [calendar_entry] ([userId], [semanticObjectType], [semanticObjectId], [calendarRevision]);
...
CREATE TABLE IF NOT EXISTS [exercise_library] (
  ...
  [mechanic] TEXT NULL,
  [category] TEXT NULL,
  [equipmentJson] TEXT NULL,
  [primaryMusclesJson] TEXT NULL,
  [secondaryMusclesJson] TEXT NULL,
  [instructionsJson] TEXT NULL,
  [video] TEXT NULL,
  [status] TEXT NOT NULL
);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_idempotency_key_scope_keyHash_unique] ON [idempotency_key] ([scope], [keyHash]);
CREATE INDEX IF NOT EXISTS [ix_idempotency_key_expiresAt] ON [idempotency_key] ([expiresAt]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_meal_revision_userId_mealId_revision_unique] ON [meal_revision] ([userId], [mealId], [revision]);
CREATE INDEX IF NOT EXISTS [ix_meal_revision_userId_createdAt] ON [meal_revision] ([userId], [createdAt]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_serving_prior_observation_serving_prior_owner_obs_unique] ON [serving_prior_observation] ([ownerUserId], [observationId]);
CREATE UNIQUE INDEX IF NOT EXISTS [ix_serving_prior_observation_serving_prior_source_rev_unique] ON [serving_prior_observation] ([ownerUserId], [sourceMealId], [sourceMealRevision], [identityKey]);
CREATE INDEX IF NOT EXISTS [ix_serving_prior_observation_serving_prior_owner_ident_confirmed] ON [serving_prior_observation] ([ownerUserId], [identityKey], [confirmedAt]);
CREATE INDEX IF NOT EXISTS [ix_serving_prior_observation_serving_prior_owner_confirmed] ON [serving_prior_observation] ([ownerUserId], [confirmedAt]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_target_revision_user_effective_revision_unique] ON [target_revision] ([userId], [effectiveDate], [revision]);
CREATE INDEX IF NOT EXISTS [ix_target_revision_userId_effectiveDate] ON [target_revision] ([userId], [effectiveDate]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_watch_delivery_delivery_user_summary_rev_uq] ON [watch_delivery] ([userId], [deviceId], [summaryId], [revision]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_watch_receipt_receipt_user_summary_rev_outcome_uq] ON [watch_receipt] ([userId], [deviceId], [summaryId], [revision], [outcome]);
CREATE INDEX IF NOT EXISTS [ix_watch_receipt_userId_updatedAt] ON [watch_receipt] ([userId], [updatedAt]);
...
CREATE UNIQUE INDEX IF NOT EXISTS [ix_workout_session_revision_sessionId_revision_unique] ON [workout_session_revision] ([sessionId], [revision]);
CREATE UNIQUE INDEX IF NOT EXISTS [ix_workout_session_revision_sessionId_idempotencyKey_unique] ON [workout_session_revision] ([sessionId], [idempotencyKey]);
CREATE INDEX IF NOT EXISTS [ix_workout_session_revision_sessionId_occurredAt] ON [workout_session_revision] ([sessionId], [occurredAt]);
