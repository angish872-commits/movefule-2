package com.movefuel.mufil2.ui.screens.fno

import androidx.compose.runtime.Composable
import androidx.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import com.movefuel.mufil2.ui.components.*
import com.movefuel.mufil2.ui.design.MoveFuelColors
import com.movefuel.mufil2.ui.design.MoveFuelTheme
import com.movefuel.mufil2.ui.navigation.MoveFuelRoute

@Composable
fun FNO004ExpandedNutritionScreen(onNavigate: (MoveFuelRoute) -> Unit) {
    MFScreenFrame(
        id = "FNO_004",
        title = "Expanded Nutrition",
        subtitle = "Confirmed nutrition for the selected day.",
        primaryLabel = "Continue",
        primaryRoute = MoveFuelRoute.FNO_005,
        secondaryLabel = "Back",
        secondaryRoute = MoveFuelRoute.FNO_003,
        onNavigate = onNavigate,
    ) {
            MFMacroBars()
            MFMetricRow("Fiber" to "24g","Sodium" to "1.8g","Coverage" to "92%")
    }
}

@Preview(showBackground = true, backgroundColor = 0xFF0E130F, widthDp = 390, heightDp = 844)
@Composable
private fun FNO004ExpandedNutritionScreenPreview() {
    MoveFuelTheme { FNO004ExpandedNutritionScreen {} }
}
